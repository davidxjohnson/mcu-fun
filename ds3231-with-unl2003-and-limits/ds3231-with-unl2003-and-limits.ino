...
#define LIMIT_SWITCH_PIN 4 // button push interrupt pin - also known as D4 on Nano
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(RTC_SQW_PIN), checkLimitSwitch, CHANGE);
...
void loop() {
  motorTurn (STEPPER_STEPS_REQ); // turn valve on
  sleepTime();
...
void motorPowerOff () {
  ...
  pinMode(STEPPER_PIN1, INPUT);
  digitalWrite(STEPPER_PIN2, LOW);
  pinMode(STEPPER_PIN2, INPUT);
  digitalWrite(STEPPER_PIN3, LOW);
  pinMode(STEPPER_PIN3, INPUT);
  digitalWrite(STEPPER_PIN4, LOW);
  pinMode(STEPPER_PIN4, INPUT);
}
